...
class DhanFoodLossScreen extends StatefulWidget {

  const DhanFoodLossScreen({super.key});

  @override
  State<DhanFoodLossScreen> createState() => _DhanFoodLossScreenState();
}

class _DhanFoodLossScreenState extends State<DhanFoodLossScreen> {
  ...
  void _initializeLossKeys() {
    ...
    var module4List = [];
    module4List = providerDhan.member.module4ModelList ?? [];
  ...
  Widget build(BuildContext context) {
    ...
    String cropName = 'ধান (সকল ধরনের ধান)';

    String title = '$cropName (গত এক বছরে)';

    return Consumer<DhanProvider>(builder: (context,provider,child)=>Scaffold(
      appBar: AppBar(
        leading: IconButton(
          onPressed: () {
