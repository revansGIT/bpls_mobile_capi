...
class CarpFoodLossScreen extends StatefulWidget {

  const CarpFoodLossScreen({super.key});

  @override
  State<CarpFoodLossScreen> createState() => _CarpFoodLossScreenState();
}

class _CarpFoodLossScreenState extends State<CarpFoodLossScreen> {
  ...
  void _initializeLossKeys() {
    var module4List = [];
    module4List = providerCarp.member.module4ModelList ?? [];
  ...
  Widget build(BuildContext context) {
    ...
    // CarpProvider provider = Provider.of<CarpProvider>(context);
    String cropName = 'কার্প জাতীয় মাছ (রুই, কাতলা, কালবাউশ, মৃগেল, অন্যান্য কার্প)';

    String title = '$cropName (গত এক বছরে)';

    return Consumer<CarpProvider>(builder: (context,provider,child)=>Scaffold(
      appBar: AppBar(
        leading: IconButton(
          onPressed: () {
